...
int translation(const char *istr) {
  ...
  while (istr[ipos] != '\0') {
    ...
    if (prevchar - istr[ipos] < 13 && prevchar - istr[ipos] > 0) {
      for (uint8_t i = 0; i < prevchar - istr[ipos]; i++) {
        putchar('-');
      }
      putchar('.');
      prevchar = istr[ipos];
      ...
      for (uint8_t i = 0; i < istr[ipos] - prevchar; i++) {
        putchar('+');
      }
      putchar('.');
      prevchar = istr[ipos];
     ...
     * calculation
     */
    for (uint8_t i = 10; i > 1; i--) { // composite numbers
...
int main(void) {
  fputs("txtbf: Enter any text:\n", stdout);
  char tmpc[2] = "0\0";
  char *input = NULL;
  int32_t strsize = 8; /* starting size */
  int32_t tmp = EOF;

  if ((input = (char *)calloc(strsize, sizeof(char))) == NULL) {
    fputs("txtbf: Cannot create input string.", stderr);
    exit(EXIT_FAILURE);
  }
  while (tmp) {
    tmp = fgetc(stdin);
    ...
    if (strlen(input) == strsize - 1) {
      ...
      if ((input = (char *)realloc(input, strsize)) == NULL) {
        fputs("txtbf: Reallocation failed.\n", stderr);
        exit(EXIT_FAILURE);
      }
    }
    tmpc[0] = (char)tmp;
  ...
  translation(input);
  free(input);
  fputs("\ntxtbf: The text has been translated into Brainfuck code.\n", stdout);
  return EXIT_SUCCESS;
}